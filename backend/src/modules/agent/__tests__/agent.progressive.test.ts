...
describe('agent progressive disclosure helpers', () => {
  ...
  it('expands base-read-pack automatically and deduplicates mounted tool names', () => {
    ...
    expect(toolNames).toContain('websearch_research_start')
    expect(toolNames).toContain('websearch_research_search')
    expect(toolNames).toContain('websearch_research_status')
    expect(toolNames).toContain('websearch_research_review')
    expect(toolNames).toContain('websearch_research_extract')
    expect(toolNames).toContain('websearch_research_assess')
    expect(toolNames).toContain('websearch_research_claim_create')
    expect(toolNames).toContain('websearch_research_evidence_candidates')
    expect(toolNames).toContain('websearch_research_evidence_add')
    expect(toolNames).toContain('websearch_research_claim_review')
    expect(toolNames).toContain('websearch_research_writeback')
    expect(hasAgentTool('websearch_research_start')).toBe(true)
    expect(hasAgentTool('websearch_research_review')).toBe(true)
    expect(hasAgentTool('websearch_research_writeback')).toBe(true)
  ...
  it('exposes UAPIs and Intel Center as discoverable toolbox capabilities in P0', async () => {
    ...
    expect(describePackForRouting('search-pack')).toContain('持久化多轮研究会话')
    expect(describePackForRouting('search-pack')).toContain('Claim-Evidence-Review')
    ...
    expect(system).toContain('websearch_research_start')
    expect(system).toContain('reuse its sessionId')
    expect(system).toContain('websearch_research_writeback')
