...
function getRoutingPrompt() {
  return [
    ...
    '- For multi-angle comparisons, cross-source verification, research reports, or investigations needing more than one search round, keep one persistent research session and reuse its sessionId for the entire topic.',
    '- Deep research workflow: websearch_research_start -> search focused rounds -> extract promising sources -> approve/reject results -> assess each round and follow its gap suggestions -> create atomic claims -> find/add anchored evidence -> review every claim. Only approved claims may be written back.',
    '- websearch_research_writeback persists approved claims and their immutable evidence into Wiki and PKM. It is a durable write operation and requires the active 1052 permission policy; never write pending, conflicted, or needs_review claims as verified knowledge.',
